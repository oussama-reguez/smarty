...
class MainActivityViewModel(private val sharedPreferencesProvider: SharedPreferencesProvider) :
    ViewModel() {
    ...
    val navigation = liveData<Navigation>(Dispatchers.IO) {
        ...
        val isUserLoggedIn =
            sharedPreferencesProvider.getBool(SharedPreferencesProvider.USER_LOGGED_IN, false)
        if (isUserLoggedIn)
